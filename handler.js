  ...
  try {
    ...
    try {
      ...
      if (gameglx) {
        ...
        if (!('ar' in gameglx.perfil)) gameglx.perfil.ar = 'ar'; // Definindo padrão 
    ...
    const ar = global.db.data.users[m.sender]?.language || 'ar';
...
export async function participantsUpdate({ id, participants, action }) {
  /************************
   * Opção de tradução de ar
   * 
   ***********************/
  const ar = global.db.data.chats[id]?.language || 'ar';
  const _translate = JSON.parse(fs.readFileSync(`./language/${ar}.json`))
      ...
      if (chat.welcome && !chat?.isBanned) {
        ...
        for (const user of participants) {
          ...
          try {
            ...
            text = (action === 'add' ? (chat.sWelcome || tradutor.texto1 ||'منور يحب انا نايت, @user!').replace('@subject', await m.conn.getName(id)).replace('@desc', groupMetadata.desc?.toString() || '*𝚂𝙸𝙽 𝙳𝙴𝚂𝙲𝚁𝙸𝙿𝙲𝙸𝙾𝙽*') :
              (chat.sBye || tradutor.texto2 || conn.bye || 'غادر المجموعه, @user!')).replace('@user', '@' + user.split('@')[0]);
...
export async function groupsUpdate(groupsUpdate) {
  ...
  const ar = global.db.data.chats[groupsUpdate[0].id]?.language || 'ar';
  const _translate = JSON.parse(fs.readFileSync(`./language/${ar}.json`))
  const tradutor = _translate.handler.participantsUpdate
...
export async function deleteUpdate(message) {
  ...
  const ar = datas.db.data.users[id]?.language || 'ar';
  const _translate = JSON.parse(fs.readFileSync(`./language/${ar}.json`))
...
global.dfail = (type, m, conn) => {
  const datas = global
  const ar = datas.db.data.users[m.sender].language || 'ar';
  const _translate = JSON.parse(fs.readFileSync(`./language/${ar}.json`))
